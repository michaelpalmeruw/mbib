...
\includegraphics{/data/code/mbib/docs/screenshots/#2}
\end{center}}
...
\item ~||~ takes precedence over (binds more strongly than) ~&&~. While this deviates from convention in mathematics, I find it more useful in practice; in my experience, it effectively does away with the need for using brackets (which \mbib anyway doesn't support at this time). You can, however, change the precedence in the \ini file if you wish.
\end{itemize}
...
\begin{itemize}
\item You can filter folders by name. Press \key{F9} to enter a word or phrase to search for. Any folder whose name contains the entered phrase, as well as its sub-folders, will be displayed, while all other regular folders (those below ``References'') will be hidden. Press \key{F10} to revert to the full display. 

\item You can also hide all references from view, displaying only the folders, using \key{F8}. (If you do so, folders that contain only references but no sub-folders will be displayed without the \texttt{+/-} expansion marker before their names.) In this decluttered view, navigation between folders can be much faster. Press \key{F8} again to revert to the full display. 
\end{itemize}

\noindent Finally, if you remember neither the folder's name nor its location, it may be useful to search for a pertinent reference instead, and then follow the ``Also in'' link to its location in the tree (see section \ref{sec-searching}, page \pageref{pg-also-in}). 
...
%
\footnote{If you don't want to move your PDF files there, you can change the path in the \ini file, or you can create symlinks to your actual storage locations inside \texttt{data/pdf}.}
...
%
to find it elsewhere. This is disabled by default, but you can enable it in the \ini file (change option \texttt{pdf\_locate = false} to \texttt{true}). This may be convenient if you prefer to store your PDF files not in a centralized location but closer to the projects and manuscripts you may be working on. 
...
This section will---some day, far, far into the future---describe everything, including configuration and code structure. 
