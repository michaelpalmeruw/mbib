...
You can generate an up-to-date \bibtex file on the fly by letting \mbib parse the \texttt{.aux} file of your document. Assume that in \texttt{mydocument.tex} you have declared ~\bibliography{references}~. Then, after you have latexed your document, but \emph{before} you run \bibtex, you can run the following command to create \texttt{references.bib}:
